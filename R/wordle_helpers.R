#' @title Wordle helpers
#'
#' @description \code{wordle_assist} returns 5-letter words that match
#'     included and excluded letter patterns. \code{wordle_sub} returns 5-letter words that contain
#'     the letters specified in `ltrs` and without the letters in `no`. The functions are similar; \code{wordle_sub} is easier to use and \code{wordle_assist} is more precise.
#'
#' @name wordle_helpers
#' @aliases wordle_assist wordle_sub
#'
#' @return Returns a vector of 5-letter words meeting supplied match
#'     criteria
#' @usage
#' wordle_assist(no=NULL, yes=NULL, words=words5)
#' wordle_sub(ltrs='arose', no=' ', words=words5)
#'
...
#'     the word, for example \code{ltrs='soa'}. If more than 5 letters are supplied, will return the
#'     empty set
#'
